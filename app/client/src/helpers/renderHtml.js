...
export default (
  ...
  /* assets.map(asset => asset.replace(',',''));  // extra comma appearing at end of each string */
    ...
    <html ${head.htmlAttributes.toString()}>
      <head>
        ...
        ${Object.keys(envAssets).map(
          key =>
            envAssets[key].hasOwnProperty('css')
              ? `<link href="${envAssets[key].css}" media="screen, projection" rel="stylesheet" type="text/css">`
              : ''
        )}
      </head>
      <body>
